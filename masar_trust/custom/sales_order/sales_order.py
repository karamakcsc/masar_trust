...
from erpnext.controllers.accounts_controller import (get_workflow_name , 
                                                     is_transition_condition_satisfied,
                                                     set_order_defaults,
                                                     validate_and_delete_children
                                                     )
from frappe.utils import get_link_to_form , flt ,getdate
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	sum_of_amount_before_discount = 0 
	for d in data:
		...
		if d.get("discount_amount"): 
			child_item.discount_amount =  d.get("discount_amount")
		if d.get("total_discount_amount"): 
			child_item.total_discount_amount =  d.get("total_discount_amount")
		if d.get("amount_before_discount"): 
			child_item.amount_before_discount =  d.get("amount_before_discount")
		if d.get("unit_price_before_discount"): 
			child_item.unit_price_before_discount =  d.get("unit_price_before_discount")
		if d.get("total_discount_amount"): 
			child_item.total_discount_amount = d.get("total_discount_amount")
		...
		if new_child_flag:
			...
			child_item.save()
	for d in data: 
		if d.get("rate") and d.get("qty"):
			sum_of_amount_before_discount += (
				(flt(d.get("rate")) + flt(d.get("discount_amount") or 0)) * flt(d.get("qty"))
			)
	frappe.db.set_value(parent_doctype , parent_doctype_name , 'total_amount_before_discount' , sum_of_amount_before_discount)
	parent.reload()

...
def execute(filters=None):
    ...
    for (item, item_group, description), wh_balance in item_balance.items():
        ...
        total_stock_value = sum(item_value[(item, item_group, description)])
        row = [item, item_map[item]["item_name"], item_group, description, total_stock_value]
...
def get_columns(filters):
    ...
    columns = [
        ...
        _("Item Group") + "::120",
        _("Description") + ":Data:200",  # Add Description column here
    ]
    return columns
...
def get_stock_ledger_entries(filters, items):
    ...
    query = (
        ...
        .inner_join(item)
        .on(sle.item_code == item.name)  # Corrected inner join syntax
        .where((sle.docstatus < 2) & (sle.is_cancelled == 0))
        .orderby(CombineDatetime(sle.posting_date, sle.posting_time))  # Assuming CombineDatetime is defined

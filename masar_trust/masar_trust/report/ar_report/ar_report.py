...
class AccountsReceivableSummary(ReceivablePayableReport):
	...
	def init_party_total(self, row):
		self.party_total.setdefault(row.party, row.sales_person, frappe._dict({
			"invoiced": 0.0,
			"paid": 0.0,
			"credit_note": 0.0,
			"outstanding": 0.0,
	...
	def set_party_details(self, row):
		...
		#self.party_total[row.sales_person].sales_person = row.sales_person
		# data = { user : 1, name : Max , three : 4}
		# is_admin = data.get( admin , False)
		for key in ('territory', 'customer_group', 'supplier_group', 'sales_person'):
			if row.get(key):
				self.party_total = row.get(key)

		#if row.sales_person:
			#self.party_total[row.sales_person].sales_person.append(row.sales_person)
			self.sales_person = row.get(key)
	...
	def get_columns(self):
		...
		self.add_column(_('Customer Sub'), fieldname='customer_sub')
		if self.filters.show_gl_balance:
		...
		if self.party_type == "Customer":
			...
			self.add_column(label=_('Sales Person'), fieldname='sales_person', fieldtype='Link',
